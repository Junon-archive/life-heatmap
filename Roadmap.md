...
## 다음 작업
**v2 — 신체 대시보드 구현** (`specs/05_physique_dashboard.md` 확정, R-31, D-18~D-21):
1. `lib/physique.ts` 순수 계산(시계열·회귀·예측·비율 동일-엔트리 규칙) + vitest
2. store/merge에 `physique` 스키마·마이그레이션·LWW 추가 + 테스트
3. PhysiqueView(히어로·카드 6장·추세 SVG·예측·페이스·기록 테이블) + 입력 모달 + 설정 뷰 목표 섹션
4. 헤더 3뷰 세그먼트, ＋ 버튼 뷰별 분기, E2E 검증, 배포

기타 개선 아이디어는 specs/04 §6, 05 §7 백로그 참고.
